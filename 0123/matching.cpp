...
int main(void)
{
	keypoint_matching();  // 키포인트 매칭
	good_matching();      // 키포인트 매칭 후 좋은 매칭 선별
	find_homography();    // 키포인트 매칭 및 호모그래피 계산
...
void find_homography()
{
	...
	for (Point2f pt : corners2) corners_dst.push_back(Point(cvRound(pt.x + src1.cols), cvRound(pt.y)));
