...
product_dict = {
  Product.NCPlastic.name : 0,
  Product.CPlastic.name : 0,
...
while True:
    ...
    if (ret == True) and (current_time > 1. / FPS):
      ...
      if confidence >= 97:
        if current_class == Product.NCPlastic.value:
          current_product = Product.NCPlastic
        elif current_class == Product.CPlastic.value:
          current_product = Product.CPlastic
      ...
      if(prev_product == Product.Nothing) and (current_product != Product.Nothing):
        if current_product == Product.NCPlastic:
          bot.pick_pl1()
        elif current_product == Product.CPlastic:
          bot.pick_pl2()
