...
# Load the model
model = load_model("s_keras_model.h5", compile=False)

# Load the labels
class_names = open("s_labels.txt", "r").readlines()
...
while True:
    ...
    if (ret == True) and (current_time > 1. / FPS):
      ...
      if confidence >= 97:
        if current_class == Product.Styrofoam.value:
      ...
      if (current_class == Product.Nothing.name) and (prev_product != Product.Nothing):
        if prev_product == Product.Styrofoam:
          bot.pick_st()
        elif prev_product == Product.Paper:
          bot.pick_pp()
