...
void blurring()
{
	...
	createTrackbar("Sigma", "dst", 0, 8, on_sigma, (void*)&src);     // 트랙바 설정, 0부터 8까지이지만 필터링이 적용되는 것은 1부터 8까지임.
	setTrackbarPos("Sigma", "dst", 4);                               // 처음 트랙바의 위치가 4에서 시작하도록 설정.
	setTrackbarMin("Sigma", "dst", 1);                               // 트랙바 조절의 최솟값을 1로 설정.
